package uk.ac.cam.ch.fetch.fulltext;
...
public class TetrahedronBacklog {
	...
	private static final String journalAbbreviation = "tetrahedron";
	private static final String siteUrl = "http://www.sciencedirect.com";
	private static final String currentIssueUrl = siteUrl+"/science/journal/00404020";

	private int sleepMax = 5000;

	public TetrahedronBacklog(String writeDir, String logPath) {
		...
		for (String volumeUrl : volumeUrls) {
			...
			for (String issueUrl : issueUrls) {
				...
				for (String fullTextUrl : fullTextUrls) {
					...
					String doiName = doiFile.getName();
					String filename = writeDir+File.separator+publisherAbbreviation+
					File.separator+journalAbbreviation+File.separator+yi.getYear()+File.separator+
					yi.getIssue()+File.separator+doiName+File.separator+doiName+".html";
					System.out.println("Writing article with DOI: "+doi);
					IOUtils.writeXML(articleDoc, filename);
				}
				updateLog(yi);
			}
		}
	...
	public List<String> getIssueUrls(Document volumeToc) {
		...
		for (int i = 0; i < issueUrlNodes.size(); i++)  {
			...
			//System.out.println(issueUrlNode.toXML());
	...
	public List<String> getVolumeUrls(Element volumesTable) {
		Nodes linkNodes = volumesTable.query(".//x:a", X_XHTML);
	...
	public static void main(String[] args) {
		...
		String logPath = "e:/tet-scraping/docs/download-log.xml";
		TetrahedronBacklog tet = new TetrahedronBacklog(writeDir, logPath);
		tet.fetch();
	}
}
